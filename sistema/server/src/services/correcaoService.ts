import { GabaritoProva } from '../types';
...
interface CorrecaoDetalhada {
  acertos: number;
  total: number;
  percentual: number;
  modo: 'rigoroso' | 'proporcional';
 ...
 */
export function parseCSV(fileBuffer: Buffer): RespostaAluno[] {
 ...
 * Example: "1,"A,B,C,D"
 */
export function parseGabaritoCSV(fileBuffer: Buffer): Map<number, string[]> {
  const fileContent = fileBuffer.toString('utf-8').replace(/\r/g, '');
  const linhas = fileContent.split('\n').filter((line) => line.trim());
...
export function corrigirProva(
  ...
  let acertos = 0;
  const questoesCorretas: number[] = [];
  const questoesErradas: number[] = [];
 ...
 */
export function resultadosToCSV(resultados: ResultadoCorrecao[]): string {
  // CSV header
 ...
 */
export function corrigirProvas(
  gabaritoBuffer: Buffer,
  respostasBuffer: Buffer,
  modo: 'rigoroso' | 'proporcional' = 'rigoroso'
): {
